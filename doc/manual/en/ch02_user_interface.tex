...
After \gesturespec{du} (or the Menu button), choose \bmenus{Config} to open a
full-screen tile grid of configuration actions (the former Config~1/2/3 edge
pages).  Portrait uses a $3{\times}4$ grid and landscape a $4{\times}3$ grid;
use the bottom \texttt{<} / \texttt{>} buttons to change pages and
\bmenus{Close} to dismiss.

Typical entries include System configuration, Planes, Devices, Flight Setup,
Wind, Profiles, Logger, and \bmenus{Tools}.  \bmenus{Tools} opens a further
tile grid with Data Management, Waypoint Editor, WeGlide Upload, and Replay.
See \ref{cha:configuration}, \ref{sec:glidepolar}, \ref{conf:comdevices},
\ref{sec:flight-setup}, \ref{sec:wind-setup}, \ref{sec:logger}, and
